...
func (h *photoTalkHandler) CreatePhotoTalk (c *gin.Context){
	...
	img,err:=phototalkBase64toEncode(buf.Bytes())
	if err!=nil{
	...
	imageKitURL, err := phototalkImageKit(context.Background(), img)
	if err != nil {
	...
	_, err = h.photoTalkService.CreatePhotoTalk(input, imageKitURL)
	if err != nil {
		// data := gin.H{"is_uploaded": false}
...
func phototalkBase64toEncode(bytes []byte) (string,error){
	...
	base64Encoding += phototalktoBase64(bytes)
...
func phototalktoBase64(b []byte) string {
	return base64.StdEncoding.EncodeToString(b)
}

func phototalkImageKit(ctx context.Context, base64Image string) (string, error) {
